...
echo -e "${GREEN}✅ $PLUGIN_DIR/backend/data/neo4j/data directory created${NC}"
mkdir -p "$PLUGIN_DIR/backend/data/neo4j/logs"
sleep 0.5  # Wait for directory to be created
echo -e "${GREEN}✅ $PLUGIN_DIR/backend/data/neo4j/logs directory created${NC}"
mkdir -p "$PLUGIN_DIR/backend/data/neo4j/import"
sleep 0.5  # Wait for directory to be created
echo -e "${GREEN}✅ $PLUGIN_DIR/backend/data/neo4j/import directory created${NC}"
mkdir -p "$PLUGIN_DIR/backend/data/neo4j/plugins"
sleep 0.5  # Wait for directory to be created
echo -e "${GREEN}✅ $PLUGIN_DIR/backend/data/neo4j/plugins directory created${NC}"
mkdir -p "$PLUGIN_DIR/backend/data/regulaite-files"
echo -e "${GREEN}✅ $PLUGIN_DIR/backend/data/regulaite-files directory created${NC}"
sleep 0.5  # Wait for directory to be created
...
echo -e "${GREEN}✅ $PLUGIN_DIR/backend/data/qdrant directory created${NC}"
sleep 0.5  # Wait for directory to be created
...
# Prompt MariaDB database name if not in .env
...
CREATE INDEX IF NOT EXISTS FOR (c:Chunk) ON (c.text);

CREATE CONSTRAINT IF NOT EXISTS FOR (l:Legislation) REQUIRE l.reference IS UNIQUE;
...
# Install Python pkgs that are failed to install during Docker build
if docker exec -i regulaite-ai-backend pip install celery==5.5.1 \
    redis==5.2.1 \
    flower==2.0.1; then
    echo -e "${GREEN}✅ All pkgs installed successfully${NC}"
    docker restart regulaite-ai-backend regulaite-celery-flower regulaite-celery-worker regulaite-redis
else
    echo -e "${RED}Error installing additional pkgs${NC}"
fi
...
echo -e "  docker logs regulaite-weaviate"
echo ""
echo -e "${YELLOW}You can now start the Kibana development server:${NC}"
echo -e "  yarn kbn bootstrap  # If you haven't already"
echo -e "  yarn start --run-examples"
echo ""
echo -e "${YELLOW}Navigate to your plugin at:${NC}"
echo -e "  http://localhost:5601/app/regulAite"
